...
# all overlay group names that include custom layer control .pngs
...
```{r create leaflet map - voucher gap states}
# Create leaflet palette, labels, map object
bins_vouchergap_states <- c(0, 10, 25, 50, 100, 200)
#if decide to use a colorNumeric gradient, use colorNumeric(c("#e8e9ee", "#1f2859")

palette1a <- colorBin(palette="BuPu",domain=mapdata_states$`Voucher Gap at 30% AMI, Normalized by Renter Pop`, bins=bins_vouchergap_states)
palette1b <- colorBin(palette="BuPu", domain=mapdata_states$`Voucher Gap at 50% AMI, Normalized by Renter Pop`, bins=bins_vouchergap_states)
...
map_states_voucher_gap <- map %>% setView(lng = -98.34,
  ...
  htmlwidgets::onRender("
        function() {
            $('.legend').prepend('<label style=\"text-align:left\"><strong>Voucher Gap Ratio<br>(Weighted) at</strong></label><br>');
        }
    ") %>%
  htmlwidgets::onRender("
        function() {
...
```{r create leaflet map - voucher gap for msas}
# Create leaflet palette, labels, map object
bins_vouchergap_msas <- c(0, 25, 50, 100, 500, 1000, 2750)

palette2a <- colorBin(palette="BuPu", domain=mapdata_msas$`Weighted Voucher Gap 30AMI`, bins=bins_vouchergap_msas)
palette2b <- colorBin(palette="BuPu", domain=mapdata_msas$`Weighted Voucher Gap 50AMI`, bins=bins_vouchergap_msas)
...
# Map Set 2: States, MSAs, and Rural Areas - Expected Voucher Use
...
```{r create leaflet map for voucher eligiblity and use - states}
# Create leaflet palette, labels, map object
states_use_bins <- c(0, 25000, 50000, 75000, 100000, 125000, 150000, 175000, 200000, 850000)
...
```{r create leaflet map for voucher eligiblity and use - msas}
# Create leaflet palette, labels, map object
msa_bins <- c(0, 10000, 20000, 30000, 40000, 50000, 100000, 150000, 200000, 350000)
msa_palette30 <- colorBin("Greens", mapdata_msas$`Total Households 30AMI Expected`, bins = msa_bins)
msa_palette50 <- colorBin("Greens", mapdata_msas$`Total Households 50AMI Expected`, bins = msa_bins)
...
```{r map voucher eligibility and use - rural areas mapped as states}
# Create leaflet palette, labels, map object
...
```{r popup tables for poverty at 30AMI, eval=FALSE, include=FALSE}
#DON'T RUN THIS - THIS WAS FOR SOME PREVIOUS TESTING
...
#"#7e4e90ff",
pov_bins_total <- c(0, 5000, 10000, 25000, 50000, 125000, Inf)
pov_bins <- c(0, 5000, 10000, 25000, 50000, 125000, Inf)
pal_total <- colorBin(palette=pal_total_gradient, domain=mapdata_poverty$`Total Households Lifted Out of Poverty Expected 30AMI`, bins = pov_bins_total)
pal_children <- colorBin(palette=blue_grad, domain=mapdata_poverty$`Households with Children Lifted Out of Poverty Expected 30AMI`, bins = pov_bins)
pal_seniors <- colorBin(palette=purple_grad, domain=mapdata_poverty$`Households with Seniors (62+) Lifted Out of Poverty Expected 30AMI`, bins = pov_bins)
pal_ofcolor <- colorBin(palette=orange_grad, domain=mapdata_poverty$`Households of Color Lifted Out of Poverty Expected 30AMI`, bins = pov_bins)
...
# Create leaflet palette, labels, map object
pal_children_grp <- colorBin(palette=blue_grad, domain=mapdata_states$`Households with Children 30AMI Expected`)
pal_seniors_grp <- colorBin(palette=purple_grad, domain=mapdata_states$`Households with Seniors (62+) 30AMI Expected`)
pal_ofcolor_grp <- colorBin(palette=orange_grad, domain=mapdata_states$`Households of Color 30AMI Expected`)
