...
## Read in the csv
mapdata <- read.csv("data/voucher_map_data_states_final.csv")
...
msas_mapdata <- read.csv("data/voucher_map_data_msas_final.csv")
...
```{r creating popup bar graphs for the voucher gap map}
#ONLY DOING THIS FOR 30% AMI? eligible hh we have for each scenario, but only calculated voucher gap at 30 ami.

mapdata_states_long <- mapdata %>%
  dplyr::select(State, Actual_Vouchers_Avail, Eligible_HH_30AMI, Voucher_Gap_30AMI) %>%
  dplyr::rename("Actual Vouchers Available in 2020" = Actual_Vouchers_Avail) %>%
  dplyr::rename("Total Eligible Households" = Eligible_HH_30AMI) %>%
  dplyr::rename("Voucher Gap" = Voucher_Gap_30AMI)

mapdata_states_long <- melt(mapdata_states_long, id="State", measure=c("Actual Vouchers Available in 2020", "Total Eligible Households", "Voucher Gap"))

mapdata_states_long_labels=c("Actual\nVouchers\nAvailable\nin 2020", 
                        "Total\nEligible\nHouseholds", "Voucher\nGap")

my_list <- list()  
loop<-for (i in unique(mapdata$State)) {
State<-mapdata_states_long%>% filter(State==i)
  plot<-ggplot(State, aes(x=variable, y=value, fill=variable)) + 
    geom_bar(stat="identity", show.legend = FALSE) +
    geom_text(aes(label = format(value, big.mark = ",", scientific = FALSE)), vjust=-0.4, color="black", size=4) +
    scale_fill_manual(values = paletteHIP3) + 
    scale_y_continuous(labels = scales::comma_format(),limits=c(0, 1450000)) +
    scale_x_discrete(labels=mapdata_states_long_labels)+
    labs(x="", y="", title=i)+
    plotTheme()
  my_list[[i]] <- plot
}
...
map.states_voucher_gap <- map %>% setView(lng = -98.34,
                lat = 39.49,
                zoom = 4.25) %>%
  ...
  addPolygons(data = mapdata,
              fillColor = ~ palette1a(Voucher_Gap_30AMI),
              stroke = T,
              weight=0.5,
              color="grey",
              fillOpacity = 1,
              group = "Voucher Gap",
                ...
                fillOpacity = 1,
                bringToFront = T)) %>%
              # popup = addPopupGraph(my_list, tooltip = TRUE)
  addPopupGraphs(
    my_list,
    group="Voucher Gap",
    opacity = 1,
  ...
  addLegend(data=mapdata,
            ...
            values = ~Voucher_Gap_30AMI, 
            pal = palette1a,
            opacity = 1,
            title = "Voucher Gap",
            group = "Voucher Gap")

map.states_voucher_gap
#saveWidget(map.states_voucher_gap, file="map_states_voucher_gap_draft_july15.html")

```

# Map Set 2: States and MSAs
## States
```{r popup bar graphs for states at 30AMI}
mapdata_states_long_30 <- mapdata %>%
  dplyr::select(State, Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.x, Number.of.Vouchers.NUMERIC.x) %>%
  dplyr::rename("Total Eligible Households" = Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.x) %>%
  dplyr::rename("Total Households Served" = Number.of.Vouchers.NUMERIC.x)

mapdata_states_long_30 <- melt(mapdata_states_long_30, id="State", measure=c("Total Eligible Households", "Total Households Served"))
...
mapdata_states_long_50 <- mapdata %>%
  dplyr::select(State, Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.y, Number.of.Vouchers.NUMERIC.y) %>%
  dplyr::rename("Total Eligible Households" = Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.y) %>%
  dplyr::rename("Total Households Served" = Number.of.Vouchers.NUMERIC.y)

mapdata_states_long_50 <- melt(mapdata_states_long_50, id="State", measure=c("Total Eligible Households", "Total Households Served"))
...
State<-mapdata_states_long_50 %>% filter(State==i)
  plot<-ggplot(State, aes(x=variable, y=value, fill=variable)) + 
    ...
    geom_text(aes(label = format(value, big.mark = ",", scientific = FALSE)), vjust=-0.4, color="black", size=4) +
    scale_fill_manual(values = paletteHIP2) + 
    scale_y_continuous(labels = scales::comma_format(),limits=c(0, 2450000)) +
...
map.states_vouchers_scenarios <- map %>% setView(lng = -98.34,
                lat = 39.49,
                zoom = 4.25) %>%
  setMaxBounds(lat1=5.499550, lng1=-167.276413, lat2=83.162102, lng2=-52.233040) %>%
  addMapPane(name = "polygons", zIndex = 410) %>% 
...
msas_mapdata_long_30 <- msas_mapdata %>%
  dplyr::select(MSA, Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.x, Number.of.Vouchers.NUMERIC.x) %>%
  dplyr::rename("Total Eligible Households" = Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.x) %>%
  dplyr::rename("Total Households Served" = Number.of.Vouchers.NUMERIC.x)

msas_mapdata_long_30 <- melt(msas_mapdata_long_30, id="MSA", measure=c("Total Eligible Households", "Total Households Served"))

msas_mapdata_long_30_labels=c("Total\nEligible\nHouseholds", "Total\nHouseholds\nServed")

plot_list_msas_30AMI <- list()  
loop<-for (i in unique(msas_mapdata$MSA)) {
...
msas_mapdata_long_50 <- msas_mapdata %>%
  dplyr::select(MSA, Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.y, Number.of.Vouchers.NUMERIC.x) %>%
  dplyr::rename("Total Eligible Households" = Number.of.Vouchers.if.Only.Renters.Participate.NUMERIC.y) %>%
  dplyr::rename("Total Households Served" = Number.of.Vouchers.NUMERIC.x)

msas_mapdata_long_50 <- melt(msas_mapdata_long_50, id="MSA", measure=c("Total Eligible Households", "Total Households Served"))
...
map.msas_vouchers_scenarios <- map %>% setView(lng = -98.34,
                lat = 39.49,
                zoom = 4.25) %>%
  setMaxBounds(lat1=5.499550, lng1=-167.276413, lat2=83.162102, lng2=-52.233040) %>%
  addMapPane(name = "polygons", zIndex = 410) %>% 
...
#map.msas_vouchers_scenarios
```

```{r save widgets}
# update with today's date, if desired
saveWidget(map.states_voucher_gap, file="map.states_voucher_gap_30AMI.html")
saveWidget(map.states_vouchers_scenarios, file="map.states_vouchers_scenarios.html")
saveWidget(map.msas_vouchers_scenarios, file="map.msas_vouchers_scenarios.html")
```
